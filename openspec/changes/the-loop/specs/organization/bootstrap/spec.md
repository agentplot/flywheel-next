...
merges, and make worktrees only for places and the operator's bolt places (205).
The layout on disk SHALL be the profile's, and a host that finds a hand-made
layout SHALL refuse to start and SHALL say what differs (205).
...
nowhere else (207). A self-managed organization SHALL use its own App, its key
placed by the operator and never by an agent (207a).
...
Initialization and repository creation SHALL stamp the version they used, and
upgrading a repository's template SHALL be a chore (208).
...
#### Scenario: An upgrade is proposed, not performed
- **WHEN** a newer set exists than the one a repository was stamped with
- **THEN** upgrading it is a chore the operator accepts, and nothing is upgraded
  on its own (208)
