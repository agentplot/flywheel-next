...
- [ ] 2.6 Implement the scenario runner's core — load a scenario file, validate it against `conformance/schema.json`, seed `given` (objects, evidence, register, marks, script, files, hosts), run the `when` steps (`tick`, `response`, `evidence`, `script`, `notify`, `restart`, `direct`, `files`, `clock`, `host`), and assert `then`; verify `flywheel scenario run conformance/scenarios/S01.yaml` passes (94, `scenarios/conformance`)
