...
script, and the page. The definitions under `definitions/` are copied
...
```
cargo run -- seed scenarios/rail-mockup.yaml   # seed the state store from a scenario
cargo run -- tick                              # tick until nothing fires
cargo run -- respond 416 yes                   # answer a numbered decision, then settle
cargo run -- dictate unit/atlas/rail-tail drop # a dictation on an object
cargo run -- rail                              # print the rail
cargo run -- log 60                            # the machinery log
cargo run -- serve --port 4242                 # the page on http://localhost:4242
