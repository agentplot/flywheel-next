...
// ---- 16.5a the scenarios a two-host run is for, asserted

/// S13, S17 and S18 under `--hosts real`, through the assertions rather than
/// past them. Each is a scenario about two hosts on one shared line — a lost
/// host and a takeover, a race for one lease, a route cut for an hour — so the
/// runner holds no engine and every clause of their `then:` is answered from
/// the run record and the store the child hosts wrote (147, 150, 151, 162,
/// 163, 165, I11, I15, D15).
#[test]
fn the_two_host_scenarios_pass() {
    std::env::set_var(BINARY_ENV, flywheel_binary());
    ...
    for name in ["S13", "S17", "S18"] {
        let path = root().join(format!("conformance/scenarios/{name}.yaml"));
        let outcome = conformance::run_one(&path, &options);
        assert_eq!(
            outcome.status,
            conformance::Status::Passed,
            "{name} under `--hosts real`: {:?} {}",
            outcome.reason,
    ...
    for _ in 0..2 {
        ...
        for _ in 0..5 {
            ...
            running.push(std::thread::spawn(move || {
                ...
                // A run's directories outlive the run itself so what it wrote
                // can be read; a test that plays one directly clears its own.
