...
use flywheel_sessions_operator::{evidence, session_fact, OperatorSessions};
...
fn no_agent_started() {
    ...
    let store = Arc::new(Mutex::new(sandbox(&base, "mac-mini", now).unwrap()));
...
fn idle_offers_nothing() {
    ...
    let store = Arc::new(Mutex::new(sandbox(&base, "mac-mini", start).unwrap()));
