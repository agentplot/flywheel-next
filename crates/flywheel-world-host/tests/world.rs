...
fn adopt_is_idempotent() {
    ...
    let head = flywheel_world_host::git::Repo::at(&blueprints)
        .git(&["rev-parse", "main"])
        .unwrap();
    ...
    assert_eq!(
        head,
        flywheel_world_host::git::Repo::at(&blueprints)
            .git(&["rev-parse", "main"])
            .unwrap(),
    ...
    flywheel_world_host::git::checkout_line(&blueprints, &checkout, "main").unwrap();
    let repo = flywheel_world_host::git::Repo::at(&checkout);
    ...
    let bare = flywheel_world_host::git::Repo::at(&blueprints);
    assert!(
        flywheel_world_host::git::show(&bare, "main", "theirs/notes.md")
            .unwrap()
            .is_some(),
        "adoption rewrites nothing of theirs"
    );
    assert!(
        flywheel_world_host::git::show(&bare, "main", "context-map/current.yaml")
            .unwrap()
            .is_some(),
...
fn create_state_repository() {
    ...
    let bare = flywheel_world_host::git::Repo::at(sandbox.git_host().join("willdan-state.git"));
    let paths = flywheel_world_host::git::ls_tree(&bare, "main", "").unwrap();
...
fn join_clones_only_what_is_missing() {
    ...
    for name in ["flywheel-state", "flywheel-blueprints"] {
        assert!(flywheel_world_host::git::Repo::at(world.bare(name)).is_bare());
...
fn token_written_nowhere_else() {
    ...
    for (name, _) in world.manifest.all_repositories() {
        let bare = flywheel_world_host::git::Repo::at(world.bare(&name));
        if !bare.exists() {
            continue;
        }
        for path in flywheel_world_host::git::ls_tree(&bare, "main", "").unwrap() {
            let text =
                flywheel_world_host::git::show(&bare, "main", &path).unwrap().unwrap_or_default();
