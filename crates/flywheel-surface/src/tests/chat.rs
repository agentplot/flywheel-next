...
use crate::chat::{Chat, Heard, Message, Recorded};
...
fn chat_and_page_show_one_number() {
    let (mut store, world, defs, chat) = a_chat("one-number");
    ...
    let page = crate::page::read(&mut store, &world, &defs, ADDRESS, "chuck").expect("the page");
...
fn only_a_decision_line_is_answerable() {
    let (mut store, _world, defs, chat) = a_chat("answerable-lines");
    a_decision(&mut store, &defs, "bolt/atlas/plan-rows");
    ...
    // A signal with no move raises nothing: curation moves it, or the
    // operator's hand on its capture does (19a).
    assert!(
        !post.lines.iter().any(|l| l.object == "signal/s1"),
        "an unmoved signal stands on the rail as a decision (19a): {:?}",
        post.lines
    );
}
...
fn the_reply_grammar_is_a_grammar_and_not_an_interpreter() {
    use crate::chat::{read_grammar, Grammar};
    assert_eq!(
...
fn yes_all_expands_per_decision() {
    let (mut store, mut world, defs, mut chat) = a_chat("yes-all");
    a_decision(&mut store, &defs, "bolt/atlas/plan-rows");
    a_decision(&mut store, &defs, "bolt/atlas/drop-the-tail");
    let standing = commands::rail(&mut store, &defs).expect("the rail");
...
fn one_of_a_group_is_answered_alone() {
    let (mut store, mut world, defs, mut chat) = a_chat("one-alone");
    a_decision(&mut store, &defs, "bolt/atlas/plan-rows");
    a_decision(&mut store, &defs, "bolt/atlas/drop-the-tail");
    let standing = commands::rail(&mut store, &defs).expect("the rail");
...
fn free_text_writes_nothing() {
    let (mut store, mut world, defs, mut chat) = a_chat("free-text");
    a_decision(&mut store, &defs, "bolt/atlas/plan-rows");
...
fn posted_message_carries_controls_and_link() {
    let (mut store, mut world, defs, mut chat) = a_chat("controls-and-link");
    a_decision(&mut store, &defs, "bolt/atlas/plan-rows");
...
&defs, &Message::new("1700", "chuck", &format!("{number}: yes")))
    ...
    let page = crate::page::read(&mut store, &world, &defs, ADDRESS, "chuck").expect("the page");
    let html = crate::page::render(&page);
...
fn mark_advances_with_delivery() {
    let (mut store, _world, defs, mut chat) = a_chat("mark-advances");
    a_decision(&mut store, &defs, "bolt/atlas/plan-rows");
...
fn each_sinks_tail_is_its_own() {
    let (mut store, _world, defs, mut chat) = a_chat("own-tail");
    ...
    let close = |store: &mut FakeStore, id: &str| {
        let when = store.now();
    ...
    let then = store.now() + chrono::Duration::minutes(1);
    store.set_now(then);
...
fn forward_writes_capture_and_signal() {
    let (mut store, mut world, defs, mut chat) = a_chat("forward");
    ...
    assert_eq!(captures[0].source, crate::chat::FORWARD);
    assert_eq!(
