...
fn embedded_set_matches_definitions_dir() {
    let on_disk = crate::set::digest_of_dir(&definitions_dir())
...
fn an_unbuilt_edit_is_a_different_set() {
    ...
    assert_eq!(
        crate::set::digest_of_dir(&dir).unwrap(),
        crate::set::digest(),
...
fn every_core_machine_carries_a_version() {
    let versions = crate::set::versions().expect("the embedded set parses");
    ...
    assert!(!crate::set::SET_VERSION.is_empty());
}
