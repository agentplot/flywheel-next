...
use crate::{evidence, RecordedWorkspace};
use serde_json::json;
...
fn place_advances_without_a_repository() {
    let base = dir("place");
    let _now = Utc.with_ymd_and_hms(2026, 1, 1, 0, 0, 0).unwrap();
    let mut store = FakeStore::new("mac-mini");
...
fn ordinals_gate_the_slot() {
    ...
    let seed = |store: &mut FakeStore, id: &str, ordinal: u64, state: &str| {
